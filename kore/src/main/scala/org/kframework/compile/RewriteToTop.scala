...
object RewriteToTop {
  def toLeft(rewrite: K): K = rewrite match {
    ...
    case t: KSequence => new KSequence(t map toLeft, t.att)
  ...
  def toRight(rewrite: K): K = rewrite match {
    ...
    case t: KApply => compactInjections(KApply(t.klabel, immutable(t.klist) map toRight, t.att))
    case t: KSequence => new KSequence(t map toRight, t.att)
    case t: KAs => t.alias
    case other => other
  }
  ...
  def hasRewrite(k: K): Boolean = k match {
    ...
    case t: KSequence => immutable(t).foldLeft(false)((b,k) => b || hasRewrite(k))
    case other => false
  }
